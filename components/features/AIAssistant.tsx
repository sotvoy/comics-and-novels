...
export default function AIAssistant({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }} 
      exit={{ opacity: 0 }}
      className={`fixed inset-0 bg-black/90 z-50 flex ${isExpanded ? 'items-center justify-center' : 'items-center justify-center'}`}
      onClick={onClose}
    >
      <motion.div 
        initial={{ y: '100%' }}
        ...
        className={`bg-gray-900 w-full flex flex-col ${isExpanded ? 'h-screen max-w-4xl rounded-0' : 'rounded-t-2xl h-[60vh] max-w-lg mt-20'}`}
